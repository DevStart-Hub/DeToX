...
def get_psychopy_pos_from_trackbox(win, p, units=None):
    ...
    Tobii TBCS coordinates are in range [0, 1] and are centered at the top-left
    corner of the screen. To convert them to PsychoPy coordinates, we need to
    subtract from 1 to flip the y-axis. If we want to use a different unit
    system (e.g. pixels, centimeters, degrees), we need to convert the TBCS
    coordinates to pixels first, and then perform the conversion to the desired
    unit system.
    ...
    units : str, optional
        The units for the PsychoPy coordinates. Default is None, which uses the
        window's default units.
    ...
    if units == "norm":
        ...
        # and convert to PsychoPy normalized coordinates
        return (-2 * p[0] + 1, -2 * p[1] + 1)
    elif units == "height":
