...
Your eye-tracking data is now being collected continuously and will be later saved in a HDF5 format, which is ideal for storing large datasets efficiently. We are still working on a dedicated data format guide, but DeToX stores recordings in HDF5 so gaze samples, events, and metadata can be inspected and analysed after the experiment.
